...
<?php
    if (isset($_POST['upload'])) {
        ...
        $folder_path = "forUpdate/";
        $filename = "$facultyemail." . basename($_FILES["cvfile"]["name"]);
        $newname = $folder_path . $filename;
        $FileType = pathinfo($newname,PATHINFO_EXTENSION);
        if ( password_verify($_POST['password'], $faculty['password']) ) {
            ...
            if ($FileType == "doc" || $FileType == "docx") {
                ...
                if (move_uploaded_file($_FILES["cvfile"]["tmp_name"], $newname)) {
                    if ($mysqli->query($query)) {
                        echo "<script>alert('Cv was sent Successfully!');</script>";
                        echo "<script>setTimeout(\"location.href = 'index.php';\",1500);</script>";
                    } else {
                        echo "<script>alert('Something went Wrong!');</script>";
                        echo "<script>setTimeout(\"location.href = 'updateemail.php';\",1500);</script>";
                    }
                } else {
                    echo "<script>alert('Your CV was not send! Read the guidelines before uploading!');</script>";
                    echo "<script>setTimeout(\"location.href = 'updateemail.php';\",1500);</script>";
                }
            } else {
                    ...
                    <div class="col-md-6">
                        <div class="card bg-dark">
                            <div class="card-body p-5">
                                ...
                                ?>
                                <form action='updateemail.php' class="d-inline" method='post' name='CVReplace' id='CVReplace' enctype="multipart/form-data">
